...
def check_side(image_path_list: list):
    for img_path in image_path_list:
        print(img_path)
        img = cv2.imread(img_path)
        fetch_single_hand_roi(img)


if __name__ == '__main__':
    # IMG_PATH = 'hand.jpg'
    IMG_PATH = 'right_hand.jpg'
    # IMG_PATH = 'jet_left.png'
    img_raw = cv2.imread(IMG_PATH)

    roi_draw = draw_single_hand_roi(img_raw)
    roi = fetch_single_hand_roi(img_raw)
    show_image(img_raw)
    show_image(roi_draw)
    show_image(roi)
