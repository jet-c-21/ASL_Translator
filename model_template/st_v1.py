...
def bilinear_sampler(img, x, y):
    ...
    Input
    -----
    - img: batch of images in (B, H, W, C) layout.    input_layers = layers.Input((size, size, 1))
    x = stn(input_layers)
...
# def get_stn_sequential(size):
#     return stn(input_layers)


def get_stn_model_v1(size=28):
...
def get_stn_model_v2(size=28):  # acc: 0.9261
    input_layers = layers.Input((size, size, 1))
    x = stn(input_layers)
